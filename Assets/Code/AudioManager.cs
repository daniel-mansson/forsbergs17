...
public class AudioManager : MonoBehaviour
{
	...
	void Awake()
	{
		...
		else
		{
			...
			PlayMusic("ambience", false, 0.5f);
		}
	}

	public static void Play(string name, Vector3 pos, float volume = 1f)
	{
		EventManager.Instance.SendEvent(new AudioEvent(name, pos, volume));
	}
	...
	void OnAudio(AudioEvent args)
	{
		...
		if (audio != null)
		{
			...
			var clip = audio.clips[UnityEngine.Random.Range(0, audio.clips.Count)];
